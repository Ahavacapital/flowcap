    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .tb{height:56px;min-height:56px;border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 20px;gap:12px;background:var(--bg2)}
        .ct{flex:1;overflow-y:auto;padding:20px}
        .sbl{padding:18px 18px 14px;border-bottom:1px solid var(--border)}
        ...
        .bd{background:#ef444415;color:var(--red);border:1px solid #ef444430}.bd:hover{background:#ef444425}
        .bsu{background:#10b98115;color:var(--green);border:1px solid #10b98130}.bsu:hover{background:#10b98125}
        ...
        .pt2 th{text-align:left;padding:8px 12px;font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;font-family:var(--mono);border-bottom:1px solid var(--border);font-weight:400;white-space:nowrap}
        ...
        .tab{padding:9px 14px;font-size:13px;color:var(--text3);cursor:pointer;border-bottom:2px solid transparent;transition:all .15s;margin-bottom:-1px}
        ...
        .quick-tab{padding:5px 12px;border-radius:20px;font-size:12px;cursor:pointer;border:1px solid var(--border2);background:transparent;color:var(--text3);transition:all .15s;font-family:var(--font)}
        .quick-tab:hover{color:var(--text);border-color:var(--border3)}
        .quick-tab.ac{background:var(--accent);color:#fff;border-color:var(--accent)}
        .action-bar{display:flex;gap:6px;flex-wrap:wrap;margin-top:4px}
        .note-card{padding:10px 12px;background:var(--bg3);border-radius:var(--radius);border-left:3px solid;margin-bottom:8px}
        .note-card.general{border-color:var(--text3)}
        .note-card.risk{border-color:var(--red)}
        .note-card.approval{border-color:var(--green)}
        .note-card.condition{border-color:var(--amber)}
        .note-card.followup{border-color:var(--purple)}
        .note-card.system{border-color:var(--accent)}
        ...
        .deal-card:hover{border-color:var(--border3);transform:translateY(-1px)}
        .today-badge{background:#10b98120;color:var(--green);border:1px solid #10b98140;padding:2px 8px;border-radius:10px;font-size:10px;font-family:var(--mono)}
        @keyframes fi2{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
        @keyframes spin{to{transform:rotate(360deg)}}
        .fa{animation:fi2 .2s ease forwards}
        .pulse{animation:pulse2 2s ease infinite}
        @keyframes pulse2{0%,100%{opacity:1}50%{opacity:.6}}
        @media(max-width:768px){.sb{display:none}.sg{grid-template-columns:1fr 1fr}.fg{grid-template-columns:1fr}}
...
const SL={new:'New',scrubbing:'Scrubbing',underwriting:'Underwriting',offered:'Offered',docs:'Docs Pending',contracts:'Contracts',bankverify:'Bank Verify',funded:'Funded',declined:'Declined',renewal:'Renewal'};
...
function mapDeal(d){
  return {
    ...
    sellRate:d.amount_approved?1.499:null,
    ...
    profit:d.amount_approved&&d.factor_rate?Math.round(d.amount_approved*(1.499-d.factor_rate)):null,
...
function App(){
  ...
  const syncSheets=async()=>{
    setSyncing(true);
    try{
      await fetch('/api/sheets/sync',{method:'POST',headers:{'Authorization':'Bearer '+window.__CRON_SECRET||''}});
    ...
    <>
      <div id="root">
        <div className="sb">
          ...
          {[
            ...
            <div key={n.id} className={'ni'+(pg===n.id?' ac':'')} onClick={()=>setPg(n.id)}>
              ...
              {n.b>0&&<span className={'nb'+(n.bc?' '+n.bc:'')}>{n.b}</span>}
            </div>
          ))}
          ...
          <div className={'ni'+(pg==='settings'?' ac':'')} onClick={()=>setPg('settings')}>
        ...
        <div className="mn">
          ...
          <div className="ct">
            {pg==='dashboard'&&<Dashboard deals={deals} setPg={setPg} setSel={setSel} tf={tf} tp={tp} active={active} todayDeals={todayDeals} funded={funded} declinedCount={declinedCount}/>}
            {pg==='pipeline'&&<Pipeline deals={deals} setSel={setSel} setShowNew={setShowNew} upd={upd}/>}
...
function Dashboard({deals,setPg,setSel,tf,tp,active,todayDeals,funded,declinedCount}){
  const approvalRate=deals.length>0?Math.round(funded.length/deals.filter(d=>['funded','declined'].includes(d.status)).length*100||0):0;
  const feed=[
    {t:'Auto',x:'Gmail watcher running every 5 minutes',d:'green'},
    {t:'Auto',x:'Google Sheets sync every 15 minutes',d:'green'},
    {t:'Auto',x:'AI scrubber active on all new deals',d:'green'},
  ];
  return(
    <div className="fa">
      <div className="sg">
        <div className="sc2"><div className="sl2">Funded (All Time)</div><div className="sv" style={{color:'var(--green)'}}>{f$(tf)}</div><div className="sd up">{funded.length} deals funded</div></div>
        <div className="sc2"><div className="sl2">Active Pipeline</div><div className="sv">{active.length}</div><div className="sd up">{todayDeals.length} new today</div></div>
        ...
        <div className="sc2"><div className="sl2">Approval Rate</div><div className="sv">{approvalRate||0}%</div><div className="sd dn">{declinedCount} declined</div></div>
      </div>
        ...
        <div style={{marginBottom:16,padding:'12px 16px',background:'#10b98112',border:'1px solid #10b98140',borderRadius:'var(--radius-lg)',display:'flex',alignItems:'center',gap:12}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:12,color:'var(--text3)'}}>
              {todayDeals.map(d=>d.business).join(', ')}
            </div>
          </div>
          <button className="btn bsu bs" onClick={()=>setPg('deals')}>View deals</button>
        </div>
      )}
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:14}}>
                ...
                <tr key={d.id} onClick={()=>setSel(d)}>
                  <td>
                    <div style={{fontWeight:500,color:'var(--text)',display:'flex',alignItems:'center',gap:6}}>
                      {isToday(d.submittedFull)&&<span style={{width:6,height:6,borderRadius:'50%',background:'var(--green)',display:'inline-block'}}/>}
                      {d.business}
                    </div>
                    <div style={{fontSize:11,color:'var(--text3)',fontFamily:'var(--mono)'}}>{d.id}</div>
                  </td>
                  <td style={{color:'var(--text2)',fontSize:12}}>{d.broker}</td>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          <div className="cd">
            <div style={{fontSize:13,fontWeight:500,color:'var(--text)',marginBottom:12}}>System status</div>
          ...
          <div className="cd">
            <div style={{fontSize:13,fontWeight:500,color:'var(--text)',marginBottom:10}}>Pipeline breakdown</div>
...
function Pipeline({deals,setSel,setShowNew,upd}){
  const stages=['new','scrubbing','underwriting','offered','docs','contracts','bankverify'];
  return(
    <div className="fa">
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
        <div style={{fontSize:13,color:'var(--text3)'}}>Active deals moving through your pipeline</div>
        <button className="btn bp" onClick={()=>setShowNew(true)}>+ New deal</button>
      </div>
              ...
              {sd.map(d=>(
                <div key={d.id} className="deal-card" onClick={()=>setSel(d)}>
                  <div style={{fontSize:11,fontWeight:500,color:'var(--text)',marginBottom:2,display:'flex',alignItems:'center',gap:4}}>
                    {isToday(d.submittedFull)&&<span style={{width:5,height:5,borderRadius:'50%',background:'var(--green)',display:'inline-block',flexShrink:0}}/>}
                    <span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{d.business}</span>
                  </div>
                  <div style={{fontSize:10,color:'var(--text3)',fontFamily:'var(--mono)',marginBottom:5}}>{d.id}</div>
                  {d.amount&&<div style={{fontSize:11,fontFamily:'var(--mono)',color:'var(--teal)',marginBottom:3}}>{f$(d.amount)}</div>}
                  {!d.amount&&d.requested&&<div style={{fontSize:11,fontFamily:'var(--mono)',color:'var(--text3)',marginBottom:3}}>{f$(d.requested)}</div>}
...
function AllDeals({deals,setSel,setShowNew}){
  ...
  const filt=d=>{
    ...
    if(s&&!d.business.toLowerCase().includes(s)&&!d.id.toLowerCase().includes(s)&&!d.broker.toLowerCase().includes(s)&&!d.contact.toLowerCase().includes(s))return false;
  ...
  const sortFn=(a,b)=>{
    let av,bv;
    if(sort==='submitted'){av=a.submittedFull||'';bv=b.submittedFull||'';}
    else if(sort==='risk'){av=a.risk||0;bv=b.risk||0;}
    else if(sort==='amount'){av=a.amount||a.requested||0;bv=b.amount||b.requested||0;}
    else if(sort==='profit'){av=a.profit||0;bv=b.profit||0;}
    else{av=a.business||'';bv=b.business||'';}
  ...
  const SortBtn=({col,label})=>(
    <span style={{cursor:'pointer',userSelect:'none'}} onClick={()=>toggleSort(col)}>
      {label}{sort===col?(sortDir==='asc'?' ↑':' ↓'):''}
    </span>
  );
    ...
    <div className="fa">
      ...
      {tab==='funded'&&filtered.length>0&&(
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:14}}>
          ...
          <div className="sc2" style={{padding:'12px 14px'}}><div className="sl2">Avg deal size</div><div className="sv" style={{fontSize:18}}>{f$(Math.round(filtered.reduce((s,d)=>s+(d.amount||0),0)/Math.max(1,filtered.length)/1000)*1000)}</div></div>
        </div>
      )}

      {tab==='declined'&&filtered.length>0&&(
        <div style={{marginBottom:14,padding:'10px 14px',background:'#ef444410',border:'1px solid #ef444430',borderRadius:'var(--radius-lg)',display:'flex',gap:20}}>
      ...
      <div className="cd" style={{padding:0}}>
        <div className="tw">
          <table className="pt2">
            <thead>
              <tr>
                <th><SortBtn col="id" label="Deal #"/></th>
                <th><SortBtn col="business" label="Business"/></th>
                <th>Broker</th>
                <th><SortBtn col="amount" label="Amount"/></th>
                <th>Status</th>
                <th><SortBtn col="risk" label="Risk"/></th>
                <th><SortBtn col="profit" label="Profit"/></th>
                <th>Buy / Sell</th>
                <th><SortBtn col="submitted" label="Date"/></th>
                <th>Notes</th>
              </tr>
            </thead>
            <tbody>{filtered.map(d=>(
              <tr key={d.id} onClick={()=>setSel(d)}>
                <td style={{fontFamily:'var(--mono)',fontSize:11,color:'var(--text3)'}}>
                  {d.id}
                  {isToday(d.submittedFull)&&<span style={{marginLeft:4,fontSize:9,background:'var(--green)',color:'#fff',padding:'1px 4px',borderRadius:4,fontFamily:'var(--mono)'}}>NEW</span>}
                </td>
                <td><div style={{fontWeight:500,color:'var(--text)',maxWidth:160,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{d.business}</div><div style={{fontSize:11,color:'var(--text3)'}}>{d.contact}</div></td>
                ...
                <td><span className={'badge '+sc(d.status)}>{SL[d.status]}</span></td>
                <td>{d.risk!=null?<div><span style={{fontSize:12,fontFamily:'var(--mono)',color:rc(d.risk)}}>{d.risk}</span><div className="sb2" style={{width:50}}><div className="sf" style={{width:d.risk+'%',background:rc(d.risk)}}/></div></div>:'—'}</td>
                <td style={{fontFamily:'var(--mono)',fontSize:12,color:d.profit?'var(--green)':'var(--text3)'}}>{d.profit?f$(d.profit):'—'}</td>
                <td style={{fontFamily:'var(--mono)',fontSize:11,color:'var(--text3)'}}>
                  {d.factor&&<div>{d.factor}x / 1.499x</div>}
                </td>
...
function DealModal({deal,onClose,onUpd,onRefresh}){
  ...
  else if(deal.positions>=2)flags.push({t:'amber',x:deal.positions+' positions — check stack'});
  ...
  const advance=async()=>{
    ...
    setAdvancing(true);
    try{
      const r=await fetch('/api/deals/update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({dbId:deal.dbId,status:next})});
      const data=await r.json();
      if(data.deal)onUpd({...deal,status:next});
    }catch(e){}
    setAdvancing(false);
  };
  ...
  const runScrub=async()=>{
    ...
    try{
      ...
      if(data.approved!==undefined){
        onUpd({...deal,status:data.approved?'offered':'declined',risk:data.riskScore,amount:data.approvedAmount,factor:data.buyRate,sellRate:1.499,termDays:data.termDays,profit:data.approvedAmount?Math.round(data.approvedAmount*(1.499-data.buyRate)):null});
        onRefresh();
      }
    }catch(e){}
    setScrubbing(false);
    ...
    <div className="mo" onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div className="md fa">
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:16,flexWrap:'wrap'}}>
          ...
          {isToday(deal.submittedFull)&&<span className="today-badge">Today</span>}
        ...
        {deal.status!=='declined'&&(
          <div className="ps">
            {steps.map((s,i)=>(
              <div key={s} className={'pst'+(i<idx?' dn':i===idx?' av':deal.status==='declined'?' er':'')}>
            ...
            {deal.amount&&(
              <div className="profit-box" style={{marginBottom:14}}>
                <div style={{display:'flex',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
                  <div><div className="sl2">Merchant payback</div><div style={{fontSize:16,fontFamily:'var(--mono)',color:'var(--text)'}}>{f$(deal.merchantPayback)}</div></div>
                  <div><div className="sl2">Our cost</div><div style={{fontSize:16,fontFamily:'var(--mono)',color:'var(--text)'}}>{f$(Math.round(deal.amount*deal.factor))}</div></div>
                  <div><div className="sl2">Our profit</div><div style={{fontSize:16,fontFamily:'var(--mono)',color:'var(--green)',fontWeight:600}}>{f$(deal.profit)}</div></div>
                  <div><div className="sl2">Daily payment</div><div style={{fontSize:16,fontFamily:'var(--mono)',color:'var(--amber)'}}>{f$(deal.merchantPayback&&deal.termDays?Math.round(deal.merchantPayback/deal.termDays):null)}</div></div>
                </div>
              </div>
            )}
            <div className="df">
        ...
        {tab==='underwriting'&&(
          <div>
            ...
            <div className="df" style={{marginBottom:14}}>
              ...
              <div className="dff"><div className="dl">Monthly revenue</div><div className="dv" style={{color:deal.monthlyRev>=35000?'var(--green)':deal.monthlyRev?'var(--red)':'var(--text3)'}}>{f$(deal.monthlyRev)}</div></div>
            </div>
            {flags.map((fl,i)=><div key={i} className={'rf '+fl.t}><span style={{fontSize:12,color:'var(--text2)'}}>{fl.x}</span></div>)}
            {!flags.length&&<div style={{textAlign:'center',padding:20,color:'var(--text3)',fontSize:13}}>Run scrubber to see underwriting analysis</div>}
          </div>
        )}
...
function NewDeal({onClose,onSave}){
  ...
  useEffect(()=>{
    fetch('/api/deals/list').then(r=>r.json()).then(data=>{
      const names=[...new Set((data.deals||[]).map(d=>d.broker?.name||d.contact_email).filter(Boolean))];
      setBrokers(names);
    }).catch(()=>{});
  },[]);
  ...
  const save=()=>{
    ...
    onSave({
      ...
      factor:risk>=70?1.22:risk>=60?1.29:1.38,sellRate:1.499,
      ...
      <div className="md fa">
        ...
        {step===0&&(
          <>
            <div className="fg">
              ...
              <div className="fgg"><label className="fl">Broker / ISO *</label>
                <input className="fi" placeholder="Type broker name or email" list="broker-list" value={f.broker} onChange={e=>set('broker',e.target.value)}/>
                <datalist id="broker-list">{brokers.map((b,i)=><option key={i} value={b}/>)}</datalist>
              </div>
        ...
        {step===1&&(
          <div style={{textAlign:'center',padding:'40px 0'}}>
            ...
            <div style={{fontSize:12,color:'var(--text3)'}}>Checking underwriting guidelines · NY Courts · DataMerch</div>
          </div>
        )}
        {step===2&&res&&(
          <div className="fa">
  ...
  return(
    <div className="fa">
      <div style={{display:'grid',gridTemplateColumns:'260px 1fr',gap:14}}>
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          <div style={{fontSize:12,color:'var(--text3)',marginBottom:4,fontFamily:'var(--mono)',textTransform:'uppercase',letterSpacing:'1px'}}>{brokers.length} ISO shops</div>
          {brokers.map(b=>(
          ...
          <div className="fa">
            <div className="cd" style={{marginBottom:12}}>
              <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:14}}>
                ...
                <div><div style={{fontSize:17,fontWeight:600,color:'var(--text)',fontFamily:'var(--serif)'}}>{sel.name}</div></div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
                <div className="sc2" style={{padding:'10px 12px'}}><div className="sl2">Total volume</div><div className="sv" style={{fontSize:16}}>{f$(sel.volume)}</div></div>
...
function ContractsPage({deals,upd}){
...
function MerchantPortal({deals}){
  ...
  const po=sel?Math.round((sel.balance||0)*1.02):0;
  return(
    <div className="fa">
      <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:16,padding:'12px 16px',background:'var(--bg3)',borderRadius:'var(--radius-lg)',border:'1px solid var(--border)'}}>
...
function MktPage({type}){
  const camps=type==='broker'?[
    {name:'Q3 ISO Reactivation',audience:'Inactive ISOs 90d+',status:'draft',sent:0,opens:0,clicks:0},
    {name:'New product announcement',audience:'All active ISOs',status:'draft',sent:0,opens:0,clicks:0},
  ]:[
    {name:'Renewal eligible outreach',audience:'50%+ paid merchants',status:'draft',sent:0,opens:0,clicks:0},
    {name:'Referral program',audience:'All funded merchants',status:'draft',sent:0,opens:0,clicks:0},
  ];
  return(
    <div className="fa">
      <div style={{marginBottom:16,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div style={{fontSize:13,color:'var(--text3)'}}>{type==='broker'?'Email & SMS campaigns for your ISO network':'Automated communications for funded merchants'}</div>
        <button className="btn bp bs">+ New campaign</button>
      </div>
      <div className="cd" style={{padding:0}}>
        <table className="pt2">
          <thead><tr><th>Campaign</th><th>Audience</th><th>Sent</th><th>Open rate</th><th>Click rate</th><th>Status</th><th>Action</th></tr></thead>
          <tbody>{camps.map((c,i)=>(
            <tr key={i}>
              ...
              <td><span className={'badge '+(c.status==='active'?'badge-funded':'badge-underwriting')}>{c.status}</span></td>
              <td><button className="btn bg bs">Edit</button></td>
            </tr>
          ))}</tbody>
        </table>
      </div>
